...
class Login extends Controller
{
    ...
    public function doLogin()
    {
        ...
        if($hasUser['id'] == 1 || $hasUser['groupid'] == 20){
            session('yes_admin_login',$hasUser['groupid']);
        }else{
            session('no_admin_login',$hasUser['groupid']);
        }
        ...
        if(1 != $hasUser['status']){
            writelog($hasUser['status'],$username,'用户【'.$username.'】登录失败：该账号被禁用',2);
        ...
        //获取该管理员的角色信息
        $admin = new AdminType();
        $info = $admin->getRoleInfo($hasUser['groupid'],$hasUser['id']);
        if($info == -1){
            return json(['code' => -6, 'url' => '', 'msg' => '该账号没有权限']);
        }
        ...
        writelog($hasUser['status'],session('username'),'用户【'.session('username').'】登录成功',1);
